...
def test_transfer_end_to_end(mcp, exchange_client):
    ...
    assert rd["tax_amount_usdc"] == pytest.approx(0.1)
    assert rd["net_amount_usdc"] == pytest.approx(99.9)
...
def test_bounty_claim_end_to_end(mcp, exchange_client):
    ...
    raw = mcp.build_bounty_claim_payload(
        "bridge-c1", acct_c.address, "bridge-p1", 300.0, 1 / 3
    )
    d = json.loads(raw)
    body = d["body"]
    ...
    assert rd["tax_amount_usdc"] == pytest.approx(0.1, rel=1e-4)
    assert rd["counterpart_net_usdc"] == pytest.approx(99.9, rel=1e-4)
...
def test_forged_body_after_signing_returns_401(mcp, exchange_client):
    ...
    raw = mcp.build_transfer_payload(
        "bridge-forge-s", acct_s.address, "bridge-forge-r", 50.0
    )
    d = json.loads(raw)
    body = d["body"]
